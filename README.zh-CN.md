...
**驭影**——桌面拍摄机械臂的示教与轨迹规划上位机。基于浏览器，通过串口 UART 文本协议与 ESP32
控制器通信，支持 **拖动示教 → 记录轨迹 → 平滑去噪 → 一键复现（可调速度）**，也支持在 3D
视图中摆放航点、由 jerk 受限的 S 曲线规划器经逆运动学驱动机械臂走出规划路径。
速度曲线按运镜需求调校：起步柔和、收尾干脆、过弯不顿挫。
...
![驭影 Yuying Arm Studio 界面](docs/screenshot.png)

## 功能

- **实时位姿** —— 通过 SSE 以约 20 Hz 推送真机状态，3D 视图实时跟随；右侧表格显示每轴的
  位置 / 速度 / 电流 / 力矩 / 温度 / 在线状态。
- **拖动示教** —— 电机进入零力矩模式，手掰机械臂走轨迹；后端持续采集关节角，再依次做
  角度解缠绕 → 均匀重采样 → Hampel 滤波（滑动中值 + MAD，鲁棒剔除手抖突变）→
  Savitzky-Golay 平滑（窗口内最小二乘多项式拟合，比滑动平均更贴合原曲线）。
...
- **笛卡尔遥控** —— 末端按可设步长沿 X/Y/Z 平移、绕 R/P/Yaw 旋转，经 IK 解算并可同步下发真机；
  不可达 / 奇异位姿会提示。
- **点动与标定** —— 每轴独立滑条；一键回零点；「校准零点」对齐 URDF 零位与电机绝对位置；
  可按轴翻转方向。
- **轨迹存取** —— 航点序列可导出 / 导入 JSON，支持撤销重做，以及「试行直线 / 试行曲线」
  在 3D 视图中空跑预览，确认无误再动真机。
...
4. **平滑处理** —— 调好「去毛刺/突变 (Hampel)」与「曲线平滑 (S-G)」窗口后点「平滑处理」，
   结果以青色曲线叠加。
